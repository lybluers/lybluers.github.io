...
function responsiveChart() {if(gitcalendar.simplemode){
    ...
    function getMousePos(canvas, event) {
        ...
        for (let item of gitcalendar.positionplusdata) {
            ...
            //if(0< x - item.x <lineminwitdh&&0< y - item.y <lineminwitdh){ console.log(item.count,item.date); } }} 数据统计算法 function addlastmonth() { if (gitcalendar.thisdayindex="==" 0) thisweekcore(52); thisweekcore(51); thisweekcore(50); thisweekcore(49); thisweekcore(48); gitcalendar.thisweekdatacore +="gitcalendar.firstdate[6].count;" gitcalendar.amonthago="gitcalendar.firstdate[6].date" else thisweek2core(); 1].date }; thisweek2core() for (let i="gitcalendar.thisdayindex" 1; < gitcalendar.first2date.length; i++) thisweekcore(index) item of gitcalendar.data[index]) addlastweek() gitcalendar.lastweek) gitcalendar.weekdatacore addbeforeweek() gitcalendar.beforeweek.length; addweek(data) 6) gitcalendar.aweekago="gitcalendar.lastweek[0].date;" lastweek="data.contributions[51];" 1].date; addlastweek(); fetch(githubapiurl) .then(data> data.json())
    .then(data => {
...
//手机版更换为svg绘制
if(document.getElementById("gitcalendarcanvasbox").offsetWidth</lineminwitdh&&0<>